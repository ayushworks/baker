    ...
    "backwards compatibility in serialization of case class ingredients, THIS TEST IS BROKEN FIX ME!" ignore {
      ...
      if (isWindows) Future.successful {
        ...
        val implementations = Seq(
          InteractionImplementation.unsafeFrom(shipGoodsMock),
          InteractionImplementation.unsafeFrom(sendInvoiceMock),
          InteractionImplementation.unsafeFrom(manufactureGoodsMock),
          InteractionImplementation.unsafeFrom(validateOrderMock)
        )
